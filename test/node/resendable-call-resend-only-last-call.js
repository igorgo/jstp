...
function createServer() {
  const server =
    cp.fork(path.join(__dirname, '../utils/resendable-call/server.js'));
  ...
  server.on('message', ([message, ...args]) => {
    switch (message) {
      ...
      case 'getSession':
        server.send(
          ['getSessionResponse', args[0], sessionStorage.get(args[0])]
        );
        break;
      case 'setSession':
  ...
  jstp.net.connect(appName, client, port, (error, conn, session) => {
    ...
    connection.on('error', (error) => {
      test.equal(
        ...
        'must not encounter errors other than `ECONNRESET`'
      );
    });

...
class Connection extends EventEmitter {
  constructor(transport, server, client) {
    ...
    this.on('close', this._dismantleCallbacks.bind(this));
  ...
  callMethodWithResend(interfaceName, methodName, args, callback) {
    const cb = (error, ...args) => {
      if (error === errors.ERR_CONN_CLOSED) {
  ...
  _prepareMessage(message) {
    return serde.stringify(message);
  }

  _restorePreparedMessage(preparedMessage) {
    return serde.parse(preparedMessage);
  }
  ...
  // Notify all callbacks about not being able to get a result
  //
  _dismantleCallbacks() {
    const callbacks = this._callbacks;
    this._callbacks = new Map();
    callbacks.forEach((callback) => {
      callback(errors.ERR_CONN_CLOSED);
    });
    callbacks.clear();
  }
